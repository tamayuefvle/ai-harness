...
- `MIGRATION_PENDING` のときは実装・依存追加・profile 解決済み前提の変更を開始しません。`projectId` の設定、`proposedProfiles` の選択、`npm run profile:resolve`、baseline レビュー、人間承認の案内に留めます。
...
- 新規企画: `npm run task:start`（active task が無い場合）。`brief.md` と `acceptance.md` のみ。
