...
test("canonical task states come from lifecycle manifest and keep DONE terminal",()=>{ const task=lifecycle(repoRoot,"task"); assert.deepEqual(task.activeStates,["DESIGNING","DEVELOPING","VERIFYING","REVIEWING","DEPLOY_READY"]); assert.deepEqual(task.terminalStates,["DONE"]); });
test("project lifecycle rejects skipped transitions",()=>{ assert.throws(()=>transitionFor(repoRoot,"project","PLANNING","ACTIVE"),/Invalid project transition/); });
test("project lifecycle allows MIGRATION_PENDING to PLANNING",()=>{ assert.doesNotThrow(()=>transitionFor(repoRoot,"project","MIGRATION_PENDING","PLANNING")); });
