...
export function validateCrossContracts(root = repoRoot) {
  ...
  const expectedFallback = execution.executorFallback?.defaultOrder ?? [];
  if (JSON.stringify(expectedFallback) !== JSON.stringify(["cursor","codex-cli","human"]) || execution.executorFallback?.maxBoundedStrategiesPerExecutor !== 1 || execution.executorFallback?.repeatFailedStrategy !== "deny" || execution.executorFallback?.automaticReturnToPreviousExecutor !== false) errors.push("Execution manifest fallback policy is inconsistent.");
