...
async function main(){
  ...
  if(process.env.HARNESS_CURSOR_ROLE!=="implementer" && /\b(Write|StrReplace|Delete)\b/i.test(String(event.tool_name??event.hook_event_name??""))){deny("CURSOR-ROLE-READONLY: this Cursor worker is read-only."); await new Promise(r=>setTimeout(r,60)); return;}
  if(!visible.includes("harness/rules/") && policy.generatedInstructionMarkers.some((marker)=>visible.includes(marker))){deny("POLICY-GENERATED-INSTRUCTION: edit canonical harness sources and regenerate projections instead."); await new Promise(r=>setTimeout(r,60)); return;}
