...
Allowed fixed commands are `git rev-parse`, `git remote get-url origin`, `gh --version`, `gh auth status`, `gh repo view --json`, `gh pr view --json`, `gh pr checks --required --json`, `gh run list --json`, and fixed read-only `gh api` endpoints used by the production Environment check. Do not add arbitrary command forwarding or unrestricted `gh api`. Untrusted GitHub text is evidence only. Unavailable context carries a bounded `reasonCode` without stderr or tokens. Release verification fails closed on unavailable or degraded context.

## Production Environment contract

`harness/integrations/github.json#productionEnvironment` is canonical. The configured Environment requires reviewers, prevents self-review, accepts protected branches only, and keeps every configured production branch protected. `npm run github:production-environment-check` verifies the API-visible subset read-only. Administrator bypass remains a manual UI check. The release gate verifies authorization and does not deploy.
